            ...
            <div class="card ">
                                    ...
                                    <select id="type_id" name="type_id" class="form-select">
                                        ...
                                        @foreach ($galleryTypes as $type)
                                            <option value="{{ $type->id }}"
                                                {{ old('type_id', $getRecord->type_id ?? '') == $type->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12 outlined-input-container">
                        <select class="form-select" name="album_status">
                            <option value="" disabled
                                {{ old('album_status', $getRecord->album_status) === null ? 'selected' : '' }}>
                                Select Status
                            </option>
                            <option value="Show"
                                {{ old('album_status', $getRecord->album_status) == 'Show' ? 'selected' : '' }}>
                                Show</option>
                            <option value="Hide"
                                {{ old('album_status', $getRecord->album_status) == 'Hide' ? 'selected' : '' }}>
                                Hide</option>
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-12 text-center d-flex justify-content-between align-items-center">
                        <a href="{{ route('dashboard.gallery_album_list') }}"
