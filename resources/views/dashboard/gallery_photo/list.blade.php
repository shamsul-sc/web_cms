                        ...
                        @if ($getGalleryPhoto && $getGalleryPhoto->count())
                            @forelse ($getGalleryPhoto as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->caption }}</td>
                                    <td>
                                        @if (!empty($value->image) && file_exists(public_path('uploads/gallery_photo/thumbnail/' . $value->image)))
                                            <img src="{{ asset('uploads/gallery_photo/thumbnail/' . $value->image) }}"
                                ...
                                {{ $value->status === 'Show' ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-warning' }}">
                                    ...
                                    <td class='mt-4'>
                                        ...
                                        <form id="delete-form-{{ $value->id }}"
                                            action="{{ route('dashboard.gallery_photo_deleted', $value->id) }}"
